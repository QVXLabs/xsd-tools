...
namespace {
	...
	const std::string FACETS(XSD_FACETS_DIR "/");
...
TEST(Narrowing, CTargetsUint8) {
	const std::string xsd = FACETS + "facet_uint8.xsd";
...
TEST(Narrowing, CTargetsInt16) {
	const std::string xsd = FACETS + "facet_int16.xsd";
...
TEST(Narrowing, CUint16Boundary) {
	EXPECT_TRUE(has(gen("c-json-jsonc.template", FACETS + "facet_uint16.xsd"),
	                "uint16_t"));
}
...
TEST(Facets, AttributeValidationEmitted) {
	const std::string out = gen("python-sax", FACETS + "facet_attr.xsd");
